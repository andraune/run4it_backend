...
def init_database_test_data():
	...
	workout_cat.save(commit=False)
	print("Added {0}".format(workout_cat))
	workout_cat = WorkoutCategoryModel('Fitness', False)
	workout_cat.save(commit=False)
	print("Added {0}".format(workout_cat))
	db.session.commit()
	...
	goalCatCumClimb = GoalCategoryModel('Cumulative climb', 'm', 1)
	...
	goal.save(commit=False)
	print("Added {0}".format(goal))
	goal = GoalModel(User.find_by_username('JonnyIT').profile.id, goalCatCumRun, this_month_first, next_month_first, 0, 100, 10.321)
	goal.save(commit=False)
	print("Added {0}".format(goal))
	goal = GoalModel(User.find_by_username('JonnyIT').profile.id, goalCatFitnessCount, prev_january, next_january, 0, 100, 57)
	goal.save(commit=False)
	print("Added {0}".format(goal))
	goal = GoalModel(User.find_by_username('JonnyIT').profile.id, goalCatCumClimb, prev_january - dt.timedelta(days=10), next_january, 0, 8848, 2345)
	goal.save(commit=False)
	print("Added {0}".format(goal))
	...
	workout = WorkoutModel(User.find_by_username('JonnyIT').profile.id, workout_cat_run, "Åsen run", dt.datetime.utcnow()-dt.timedelta(seconds=90000), 3000, 658, 0, 'C:/mydev/run4it_backend/run4it/uploads/gpx/test.tcx', 1)
	workout.save(commit=False)
	print("Added {0}".format(workout))
	db.session.commit()
