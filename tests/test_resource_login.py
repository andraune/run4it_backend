...
from run4it.api.user.resource import Login, LoginFresh, LoginRefresh
...
class TestLoginResource:
	...
	def test_loginfresh_user_by_post(self, api, client):
	...
	def test_loginfresh_token_generated(self, api, client):
		url = api.url_for(LoginFresh)
		...
		client.post(url, data={"email": "confirmedUser@mail.com", "password": "confirmed123" })
	...
	def test_loginfresh_user_incorrect_password(self, api, client):
		url = api.url_for(LoginFresh)
		response = client.post(url, data={"email": "confirmedUser@mail.com", "password": "incorrect" })
		response_json = get_response_json(response.data)
		assert(response.status_code == 401)
	...
	def test_get_loginfresh_not_supported(self, api, client):
		url = api.url_for(LoginFresh)
		response = client.get(url)
		assert(response.status_code == 405) # not allowed

	def test_put_loginfresh_not_supported(self, api, client):
		url = api.url_for(LoginFresh)
		response = client.put(url)
		assert(response.status_code == 405) # not allowed

	def test_delete_loginfresh_not_supported(self, api, client):
		url = api.url_for(LoginFresh)
		response = client.delete(url)
		assert(response.status_code == 405) # not allowed
