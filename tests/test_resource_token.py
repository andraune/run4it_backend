...
class TestTokenResource:
	...
	def test_get_token_logged_in(self, api, client):
		...
		assert(response_json["token_type"] in ("access", "refresh"))
	...
	def test_update_token_actually_saved(self, api, client):
		...
		response = client.put(url, data={'revoked':'True','jti':'newjti','token_type':'refresh','username':'newuser','expires':str(dt.datetime(2002, 2, 3, 16, 15, 14, 13))}, headers=get_authorization_header(token))
		response_json = get_response_json(response.data)
